...
func main() {
	...
	switch meth {
	case "Commit":
		client := commitlog.NewServiceClient(tp)
		var req commitlog.CommitRequest
		if err := jsonUnmarshaler.Unmarshal(inpr, &req); err != nil {
			log.Fatalf("json: %s", err)
		}
		rep, err = client.Commit(ctx, &req)

	case "History":
		client := commitlog.NewServiceClient(tp)
		var req commitlog.HistoryRequest
		if err := jsonUnmarshaler.Unmarshal(inpr, &req); err != nil {
			log.Fatalf("json: %s", err)
		}
		rep, err = client.History(ctx, &req)

	case "Pending":
		client := commitlog.NewServiceClient(tp)
		var req commitlog.PendingRequest
		if err := jsonUnmarshaler.Unmarshal(inpr, &req); err != nil {
			log.Fatalf("json: %s", err)
		}
		rep, err = client.Pending(ctx, &req)
