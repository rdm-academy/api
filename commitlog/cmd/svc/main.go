...
func main() {
	...
	flag.StringVar(&mongoAddr, "mongo.addr", "127.0.0.1:27017/commitlog", "Mongo database URI.")
	...
	svc, err := commitlog.NewService(tp, db)
	if err != nil {
		log.Fatal(err)
	}

	logger.Info("serving `svc.commitlog`")

	// Serve the service.
	ctx := context.Background()

	srv := commitlog.NewServiceServer(tp, svc)
