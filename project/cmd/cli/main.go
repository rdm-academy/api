...
func main() {
	...
	case "CreateProject":
		...
		rep, err = client.CreateProject(ctx, &req)

	case "GetProject":
		client := project.NewServiceClient(tp)
		var req project.GetProjectRequest
		if err := jsonUnmarshaler.Unmarshal(inpr, &req); err != nil {
			log.Fatalf("json: %s", err)
		}
		rep, err = client.GetProject(ctx, &req)
	...
	case "ListProjects":
		...
		rep, err = client.ListProjects(ctx, &req)
	...
	case "DeleteProject":
		...
		rep, err = client.DeleteProject(ctx, &req)
