...
func main() {
	...
	switch meth {
	case "Import":
		client := data.NewServiceClient(tp)
		var req data.ImportRequest
		if err := jsonUnmarshaler.Unmarshal(inpr, &req); err != nil {
			log.Fatalf("json: %s", err)
		}
		rep, err = client.Import(ctx, &req)

	case "Upload":
		client := data.NewServiceClient(tp)
		var req data.UploadRequest
		if err := jsonUnmarshaler.Unmarshal(inpr, &req); err != nil {
			log.Fatalf("json: %s", err)
		}
		rep, err = client.Upload(ctx, &req)

	case "Describe":
		client := data.NewServiceClient(tp)
		var req data.DescribeRequest
		if err := jsonUnmarshaler.Unmarshal(inpr, &req); err != nil {
			log.Fatalf("json: %s", err)
		}
		rep, err = client.Describe(ctx, &req)
