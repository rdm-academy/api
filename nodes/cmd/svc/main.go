...
func main() {
	...
	flag.StringVar(&mongoAddr, "mongo.addr", "127.0.0.1:27017/nodes", "Mongo database URI.")
	...
	svc, err := nodes.NewService(tp, db)
	if err != nil {
		log.Fatal(err)
	}

	logger.Info("serving `svc.nodes`")

	// Serve the service.
	ctx := context.Background()

	srv := nodes.NewServiceServer(tp, svc)
